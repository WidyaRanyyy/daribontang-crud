...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $rating >= 0 && $rating <= 5) {
        try {
            $stmt = $pdo->prepare("INSERT INTO foods (name, origin, description, price_range, best_place, rating) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$name, $origin, $description, $price_range, $best_place, $rating]);
            $_SESSION['success_message'] = "Makanan berhasil ditambahkan.";
            header("Location: ../dashboard.php?page=makanan");
            exit();
        } catch (PDOException $e) {
            $error = "Gagal menambahkan makanan.";
        }
    } else {
...
<head>
    ...
    <style>
        ...
        input, textarea, select { width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 6px; font-size: 1rem; }
        textarea { height: 100px; resize: vertical; }
        .btn { padding: 12px 20px; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; text-decoration: none; display: inline-block; margin-top: 10px; }
    ...
    <div class="container">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="origin" placeholder="Contoh: Bontang Kuala">
            </div>
            <div class="form-group">
                <label>Deskripsi</label>
                <textarea name="description" placeholder="Jelaskan rasa, bahan, cara penyajian..."></textarea>
            </div>
            <div class="form-group">
                <label>Harga (Rentang)</label>
                <input type="text" name="price_range" placeholder="Rp 25.000 - Rp 40.000">
            </div>
            <div class="form-group">
                <label>Tempat Terbaik</label>
                <input type="text" name="best_place" placeholder="Contoh: Warung Apung Bontang Kuala">
            </div>
            <div class="form-group">
                <label>Rating (0–5)</label>
                <input type="number" name="rating" step="0.1" min="0" max="5" value="4.5" required>
            </div>
